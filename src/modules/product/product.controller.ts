...
import { ProductNameAlreadyExist, ProductNotFound, YouNeedChangeSomething } from "./exception/product.exception";
...
export class ProductController {
  ...
  async getAll(req: Request, res: Response) {
    try {  
      const search:IProductQueryDto = req.query;   
      const resData = await this.#productService.getAll( search );
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      if(!dto.count && !dto.name && !dto.price){
        throw new YouNeedChangeSomething();
      }
